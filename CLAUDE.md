...
| `Viv.Outbox` | **发件箱（事务性消息投递）+ Inbox（消费端幂等）** — `IVivOutbox` / `OutboxStore`（Scoped，入队走 `ExecuteSqlAsync` 并入业务事务）+ `OutboxDispatcher`/`OutboxWorker`（后台投递，原子认领）+ 手写 SQL（**一次都不经过 EF**，表 `VivOutboxMessage`）。解决「写库 + 发消息」不原子：**写和待发消息进同一个本地事务**，投递交给后台。Inbox 侧 `IVivInbox` / `InboxStore`（表 `VivInboxMessage`，消息级去重与业务级幂等两族键）+ `InboxDispatcher`（按保留期清理，独立循环）。发件箱表另存 `TraceId` / `RequestTraceId` 两列溯源（投递器在后台作用域重建不出入队时的语境，只能入队时存）。**发件箱不发自建 span** —— 见下 |
| `Viv.Redis` | Redis cache — `IRedisService` with pluggable DB allocation (`DbSelectorType`)。访问失败抛 `VivConnectionException(Redis)`（API 过滤器 `-502`，客户端只回固定文案）；`DataAccessCacheBase` 读路径 catch 后回源数据库，**取锁也走 catch 后回源**（见 `Viv.Momo` 行）。写仍抛。锁续期后台任务仍只记日志后停止；**指标** `RedisMetrics`（Meter `Viv.Redis`，9 件：命令耗时 / 失败 / 连接断连 / 缓存命中 / 回源 / 取锁耗时 / 续期停转 / 释锁失败，见 `### 慢查询与指标`；**链路追踪** span 走 `VivTracing`，发在 4 个 `ExecuteRedis*` 收口上，见 `### 链路追踪（自建 span）`）|
...
| `Viv.Aspire.ServiceDefaults` | OpenTelemetry tracing/metrics, `/health` + `/alive` endpoints, service discovery, HTTP resilience；`WithMetrics` 的 `AddMeter("Viv.Xxx")` 与 `WithTracing` 的 `AddSource` 都是**手写死的名单**（见 `### 链路追踪（自建 span）`），为此它有一条指向 `Viv.Contracts` 的 ProjectReference（本项目唯一一条，之前是零） |
...
>
> **DI 注册（推荐的消费方式）**：`AddVivConfig()`（`VivConfigLoader`，静态实例模式）把每个非 null 子配置**双注册**进 DI —— `AddSingleton(T)` + `AddSingleton(IOptions<T>)`。另有 `AddVivConfigFromConfiguration()` 走 `Configure` + `IOptionsMonitor` 支持热更新，但**只注册 `IOptions<T>`、不注册 `T` 本身**（注入具体类型会解析失败）；它同样调 `Load` 写快照，所以**热更新只对 DI 路径生效，静态快照是启动时冻结的那份** —— 走 `VivEngine.VivOptions` 的静态读取点（`RequestTokenResolver.GetInternalSecret`、网关服务名判定、`ApiStartedMiddleware`）看不到刷新。领域服务/中间件/拦截器一律**构造注入**配置（如 `IOptions<NanaOptions>`、`VivInternalTokenOptions`），不要再走静态通道；静态快照只保留给 Engine 自身的静态路径。`VivEngine.SetVivOptions` 是 `internal`，只有同程序集的 `VivConfigLoader` 调得到。**`VivConfigRegistry` 已无任何调用者**（2026-09-14 全部改为 DI），仅类定义留存。

> **环境变量覆盖（走标准配置链后生效）**：`VivOptions__*` 可覆盖任意节点（如 `VivOptions__EnvOption__InternalToken`、`VivOptions__TokenOption__SecretKey`）——此前绑定绕过 IConfiguration 时 env 对 viv 配置无效。各服务 appsettings.json 目前仍直接携带凭据（迁移现状，非本次引入）；要彻底抽离需配合 env/user-secrets/Key Vault 提供程序覆盖。
...
- **Consumer:** Extend `VivConsumer<T>`, override `ReceiveMessageAsync()` — return `SubscribeResult` 指示成功或重投。基类 `HandleAsync(NanaEnvelope<T>, CancellationToken)`（Wolverine handler 约定，`Discovery.IncludeType` 显式注册）先按 `nana:{ServiceName}:{EventType}:{MessageId}` 取 Redis 锁（`IDistributedLock`，未注册则跳过）：**谁取到锁谁进业务**，fanout 下各服务 Key 不同所以各处理一份。锁 holder 优先 `envelope.Context.HolderId`，没有才回落 `MessageId`（**不用 TraceId**，避免客户端 `X-Trace-Id` 污染锁身份）。**锁 Key 由 `VivConsumer<T>.GetConsumeLockKey(envelope)` 给出**（`protected virtual`，默认就是上面那把消息级去重锁）：重写它可把锁换成业务粒度（按订单号锁，让同一订单的不同消息互斥），返回 `null` 则这条消息完全不加锁；它在上下文水合**之后**才调用，所以重写方读得到本条消息的 `_context` 与 `LockHolderContext`。别返回空字符串 —— Redis 不接受空 Key，会抛异常进死信。锁的过期时间由 `VivConsumer<T>.GetConsumeLockTime(envelope)` 给出（同样 `protected virtual`，默认 5 分钟；非正数挡回默认值并记 Warning —— 真实实现对非正数是不抛不记的取锁失败，放任下去表现成「每条消息都进死信」而看不出原因）。它管的是**崩溃兜底**不是业务耗时上限：锁取到就自动续期（`LockAutoRenewal` 在过期时间的一半续一次），业务跑多久锁都不会掉，进程被 kill 才是真场景。`DistributedLockAccessor` 把 Redis 连接失败包成 `DistributedLockException`（Inner 为 `VivConnectionException`），基类 catch 记 Warning 后抛给 Wolverine。然后将结果映射：`Success` → 确认；`Requeue` → 抛 `VivRequeueException`（走全局重试策略）；失败 → 记日志丢弃。
...
- **配置（`AddVivWolverine`，`AddViv()` 内调用）：**
  ...
  - **`opts.ServiceLocationPolicy = ServiceLocationPolicy.AllowedButWarn`（必配，别删）**：消费者依赖 `VivConsumerDependency` / `VivLocalConsumerDependency` 注入的 `IVivLocalEventBus` / `IVivUnitOfWork` / `IVivInbox`，容器里的实现是 `internal sealed`（`LocalEventBus` / `UnitOfWorkManager` / `InboxStore`），Wolverine 没法在生成的 handler 里直接 `new`，只能从作用域容器取 —— 而**这正是它们要的**：三个都是 Scoped，必须和本次消费的 `IMomoDbContext` 同作用域。Wolverine 6.x 默认 `NotAllowed`，且不是干净地报错：它先排一个取服务的 frame 再往上硬接构造 frame，接不上就抛 `Frame chain is being re-arranged`（`InvalidOperationException`，栈落在 `JasperFx.CodeGeneration.Model.MethodFrameArranger.chainFrames`），**每条消息每 30 秒炸一次、永远进不了业务代码**，日志里还有一条前置的 `InvalidServiceLocationException` 点出三个类名。该 enum 只有 `AllowedButWarn` / `NotAllowed` 两档、没有「让且闭嘴」，警告只在 codegen 时打一次。⚠️ 那条警告末尾写着「This will throw in Wolverine 6.0 when NotAllowed becomes the default」是 5.x 遗留文案（6.25.3 已经在抛了），但它点出的方向是真的 —— 若将来这档被取消，出路是把三个实现类改 public 或给依赖换形状。全仓只有这一处设 policy。
...
- **表 `VivOutboxMessage`**（业务主库，前缀 `Viv` 让它在业务库里一眼可辨是框架表；`Sql/OutboxMessage.sql` + `Sql/OutboxMessage.pg.sql` 作嵌入资源 —— **文件名与表名不绑定**，同一份文件也提交进仓库供 DBA / 迁移脚本用）：`Id`（`IdMagic.NextId()`，**不用 IDENTITY**，手写 INSERT 不依赖回填）、`MessageId`、`EventType`、`Payload`、`Status`、`RetryCount`、`NextRetryAt`、`LeaseUntil`、`OccurredAt`、`SentAt`、`LastError`、`TraceId`、`RequestTraceId`。`Status`：`0=Pending`、`1=Processing`（已认领、有租约）、`2=Sent`、`3=Failed`（耗尽重试，等人工介入）—— **数值即库里的值**，改一次就等于把所有历史行解读错（有测试钉死）。所有时间列一律 `DateTime.UtcNow` 派生（Kind=Utc）：PG 侧是 `TIMESTAMPTZ`，Npgsql 拒绝写入 Kind=Unspecified。
- **溯源两列（`TraceId` / `RequestTraceId`）只在入队那一刻取得**：投递器跑在后台作用域里，`Activity.Current` 是 `null`、请求上下文也早没了，**不存就是永久丢**。`TraceId` 来自 `Activity.Current?.TraceId`（OTel，dashboard 上给你的那个）；`RequestTraceId` 来自 **`IVivContext.TraceId`** —— 不是 `IHttpContextAccessor`：`Viv.Outbox` 是纯 `Microsoft.NET.Sdk` 看不到 ASP.NET Core，而 `VivContextMiddleware` 填的就是 `context.TraceIdentifier`，两者本就同源。**两个都存**是因为前者在 dashboard、后者在响应体与日志里，谁都能拿来反查同一行。无请求上下文时 `IVivContext.TraceId` 给的是**空串**，存成 `NULL` 而不是空串 —— 前者是「本来就没有」，后者在库里长得像「有但是空的」。两列也进了 `ClaimBatch` 的 `RETURNING`/`OUTPUT` 清单，认回来的行与库里的行一致。
- **⚠️ 给这张表加列必须补幂等 `ALTER`**：建表那段是 `IF OBJECT_ID(...) IS NULL` / `CREATE TABLE IF NOT EXISTS`，**对已存在的表整段不进**——只往 `CREATE TABLE` 里加列的话，已建过表的库永远是 `Invalid column name 'TraceId'`，而且是**首次入队才炸**。故两个 `.sql` 后面各跟一句幂等补列（SQL Server 走 `IF COL_LENGTH(...) IS NULL`，PG 走 `ADD COLUMN IF NOT EXISTS`）。`OutboxSqlTests.Columns` 是建表脚本与认领 SQL 两条一致性断言共同遍历的清单，**加列时不同步加进去那两条断言对新列整个是空的**。
...
- **`IVivInbox` / `InboxStore`**，表 `VivInboxMessage`（`(ServiceName, IdempotentKey)` 复合主键 + `AcceptedAt`）。首次 `true`、重复 `false`（靠唯一约束冲突判定，不是先查后插）。写入走 `ExecuteSqlAsync` 所以并入调用方当前事务 —— 没有外层事务时依然落库（有持久性、没有与业务写的原子性）。`ServiceName` 取入口程序集名，与 `VivConsumer` 那把消费锁的口径对齐。
- **两个重载管的是两件事**：`TryAcceptAsync(long messageId)` 是**消息级去重**，键就是 MessageId，挡的是「同一条消息被投递两次」（发件箱重试 / 租约过期重投 / `RedeliverAsync`）；`TryAcceptAsync(string idempotentKey)` 是**业务级幂等**，键由调用方给，挡的是「同一件事提交了两次」—— 这两次在投递层看是两条不相干的消息（MessageId 不同，甚至一条来自 MQ、一条来自 HTTP 重试），消息级去重对它们完全无感。`VivConsumer.TryAcceptInboxAsync` 继续走 `long` 那个重载。
- **两族键共用一张表，靠前缀分命名空间**：消息级存 `msg:{MessageId}`、业务级存 `biz:{调用方给的键}`，前缀由 `InboxStore` 加、调用方只给裸键。**不加前缀的话数值型业务键会与 MessageId 撞成同一行**（订单号 42 与 MessageId 42 的裸值都是 `42`），表现是另一件不相干的事被静默当成重复跳过，而表里两行长得一模一样、看不出问题。键长上限 200 字符（超了 DB 直接报错，不截断）；`messageId == 0` 与空/全空白业务键都直接 `false`，不进表。
- **键的 scope 是 `ServiceName`，租户不在里面** —— 表与发件箱同属框架表，刻意不实现 `ITenant`、不过 EF。多租户下**调用方要把租户自己拼进业务键**（如 `$"{tenantId}:{orderId}"`），忘了拼就是两个租户共用一把键，后一个的正当业务被静默跳过。
- **⚠️ 业务幂等必须包在事务里用**（HTTP 侧标 `[VivUnitOfWork]`）。没有外层事务时这行插入立刻落库，业务随后失败也不会撤销 —— 键被永久标成「处理过」，客户端重试**全被拒**，而现象看起来像是「系统说他处理过了」。消息级去重没这个问题：它本来就发生在「这条消息已经处理完了」之后。
- **两族共用一个保留期**：`InboxOptions.RetentionDays` 同时是消息去重窗口和业务幂等窗口。业务键通常几分钟就够，搭 7 天一起走无害（表略微大一点），不值得为此拆第二个旋钮。
...
Aspire 给的是**传输层**：OTLP 导出、dashboard、跨服务串联（`traceparent` 由 `AddAspNetCoreInstrumentation` + `AddHttpClientInstrumentation` 自动带上）。缺的是**通用库的 span** —— 一个请求进了 Apex.Api，瀑布图上只有一条 HTTP span，看不见它查了哪几次库、拧了几次 Redis。这一节补的就是这个。

源名与 `ActivitySource` 放在 **`Viv.Contracts/VivTracing.cs`**（项目根，与 `LockKeyMagic` 并排；与 `VivHeaderContract` 同一个考虑 —— 跨层共用的字符串只留一个来源）。`Viv.Contracts` 是 net10.0，`ActivitySource` 在共享框架里，**不新增任何包**，与 `Meter` / `MeterListener` 同理。

**四处 span 都包在既有的收口上，没有一处是新开的路：**

| 位置 | span | 为什么是这儿 |
|---|---|---|
| `QueryTelemetry.Record` | `db.query`，标签 `path` / `op` / `db.statement` | EF 与 Dapper 两条路的耗时都汇到这一个方法，加一次等于两条路全覆盖 |
| `VivRedis` 的 4 个 `ExecuteRedis*` 重载 | `redis.command`，标签 `op` / `result`(ok/error) | `RedisService.cs` 里几十个业务方法全从这四个过 |
| `NanaEventPublisher` 的 4 个发布方法 | `mq.publish`，标签 `event` / `delay.ms`（仅延迟版） | 四个方法就是 `IVivEventPublisher` 的全部出口，加一次等于发布侧全覆盖 |
| `Extensions.cs` 的 `.AddSource("Wolverine")` | 消费侧 span | Wolverine 自带，生产侧代码一行不用动 |

- **`db.query` 是「补记」不是「包一段」**：`Record` 在命令跑完之后才被调（EF 从 `CommandExecutedEventData.Duration` 拿耗时，Dapper 从 `Measure` 的 `Stopwatch` 拿），所以起止时间都得自己给 —— 用带 `startTime` 的重载起一个，再 `SetEndTime` 收在当下。⚠️ 两个参数的类型不一样，不是笔误：`StartActivity` 收 `DateTimeOffset`、`SetEndTime` 收 `DateTime`。**忘了往前推 `startTime` 的话 duration 恒为 0**，而 span 照样出得来、标签照样对 —— 最难看出来的一种，`QueryTelemetryTests` 里有一条专钉它。没有监听者时用 `HasListeners()` 早退，省的是白建一个 `ActivityTagsCollection`。
- **`redis.command` 的 `op` 走 `[CallerMemberName]`**：`ExecuteRedis*` 各加一个可选参数，拿到的是**调用方 `RedisService` 的方法名**（`SetAsync` / `HashGetAll` / `KeyDelete` …），编译期常量，几十个调用点一个都不用改。
- **不放 key 标签**：key 里带租户数据，写进 trace 就跟着导出到 dashboard 与任何后端了。现有 `viv.redis.command.duration` 也只有 `result` 没有 key，口径一致。
- **`mq.publish` 的 Kind 是 `Producer`**，`db.query` / `redis.command` 是 `Client` —— 瀑布图上「我们主动往外发」与「发出去等对方回」分得开。有 `Activity.Current` 时它就是父，所以 **HTTP 里发的消息挂在同一条 trace 下**（有测试钉死 `TraceId` / `ParentSpanId` 都对上）；没有（后台线程）时自成一条根 span。延迟版多一个 `delay.ms` 标签，不延迟的不带 —— 否则分不出哪些是延迟发的。
- **只有发布侧发 span，发件箱刻意不发**：`OutboxDispatcher` 跑在 `BackgroundService` 自己的作用域里，`Activity.Current` 是 `null`、与入队那一刻早已脱节，硬发一条只会是条没有来处的根 span。**溯源改走表里那两列**（见 `### Outbox（发件箱）`）。`NanaLocalEventPublisher` 也不发 —— 它走的是 Wolverine 本地队列，消费/发送两侧都已被 `AddSource("Wolverine")` 覆盖。
- **span 名是常量，区分信息走标签**，不按调用次数分配字符串、也不制造高基数 span 名。采样走默认 `ParentBased(AlwaysOn)` 全采，尾部 / 比例采样是独立一件事。

⚠️ **`AddSource` 是一份手写死的名单，与上面 `.AddMeter("Viv.Xxx")` 同形**：新起一个 `ActivitySource` 不改 `Viv.Aspire.ServiceDefaults/Extensions.cs` 就收不到，表现是「埋点明明写了，dashboard 上什么都没有」。`AddSource(builder.Environment.ApplicationName)` 那条是 Aspire 模板默认、当前收不到东西，保留 —— 主机将来按服务名发自建 span 会用上。

`Viv.Aspire.ServiceDefaults` 为此加了**本项目唯一一条 ProjectReference**（指向 `Viv.Contracts`，取 `VivTracing.SourceName`，名单写成字面量改源名时两边会静默对不上）。方向是对的（Vivian → Banshee；CLAUDE.md 那条约束是「框架不引用 Aspire」，反向不成立），也不构成循环。代价是它从此不再是通用的 Aspire 模板项目 —— 但它本来就已经只给这一个仓库用。

**不引 `OpenTelemetry.Instrumentation.EntityFrameworkCore` / `.StackExchangeRedis`**：两个都只有 beta（最新 `1.19.0-beta.1`），而仓库其余 OTel 全钉在稳定 `1.17.0`。更主要的是上一轮刚在这两处各建了一个收口，引包等于同一件事做两遍 —— `MomoMetricsCommandInterceptor` 已经是一条覆盖全部 EF 命令的 `DbCommandInterceptor`，官方那个订阅 EF 的 `DiagnosticSource`，两个并行跑会让同一条 SQL 出两份数据、两套标签口径。Redis 那个包还要求启动时就有一个活的 `IConnectionMultiplexer`，而 `RedisFactory` 是静态 `Lazy`、首次使用才建，`AddServiceDefaults()` 又跑在 `AddVivConfig()` 之前 —— 那个时点既没有 `VivOptions` 也没有 `IRedisService`。代价说清楚：自建 span 的属性**不按 OTel 语义约定**（没有 `db.system` / `db.operation`），将来接 Grafana 数据库面板或 APM 厂商时认不出来；真要接了再引那两个包也不冲突，只是会重复。

**不做（范围外）**：gRPC 客户端埋点（`AddVivGrpcClient` 目前零生产调用方，`Extensions.cs` 里那行注释留着）；框架自有语义的 span（消费锁、本地事件分发、发件箱认领 —— 那是第三层）。另外一条**既有的**口子：EF 命令抛异常时走的是 `CommandFailed` 而不是 `Executed`，所以失败的 EF 命令既不进指标也不出 span。

**测试的边界**：单测钉的是「`Record` / 发布方法发出的 span 名字 / 标签 / **时长** / **父子关系**」与「没监听者时不炸」，**不是**「`ExecuteRedis*` 真的发了 span」—— 那要连真 Redis，仓库没有那条测试基座。接线只有手工验：Aspire 起来打一个既查库又读缓存的接口，那条 HTTP span 下应该挂出若干 `db.query` 与 `redis.command`（点开一条看 `path` 是 `ef` 还是 `dapper`、**duration 不是 0**）。消费侧那条线**已经实测过**：SakuMai 定时任务发的消息，同一条 traceId 在日志里从 `Enqueued for sending ... to local://durable/` 一路串到 `Enqueued for sending NanaEnvelope\`1#... to rabbitmq://exchange/TestApexExchange`；`Wolverine.Runtime.WolverineTracing.ActivitySource.Name` 也经反射确认就是 `"Wolverine"`（不是从 DLL 字符串表猜的，不用再去试 `"Wolverine.Diagnostics"`）。⚠️ 单独 `dotnet run` 跑服务时 `OTEL_EXPORTER_OTLP_ENDPOINT` 不会被注入，**什么都不会导出**，别当成埋点没生效。
