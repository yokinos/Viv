...
namespace Viv.Momo.Sync
{
    ...
    public class SchemaSynchronizer
    {
        ...
        private readonly DatabaseSouceType _dbType;
        ...
        public async Task<List<TableInfo>> FetchActualSchemaAsync(CancellationToken cancellationToken = default)
        {
            ...
            if (_dbType == DatabaseSouceType.PostgreSQL)
            {
            ...
            else if (_dbType == DatabaseSouceType.SqlServer)
            {
        ...
        private string MapToSqlType(Type clrType, int? maxLength)
        {
            ...
            if (clrType == typeof(string))
            {
                if (maxLength.HasValue)
                    return _dbType == DatabaseSouceType.PostgreSQL ? $"varchar({maxLength})" : $"nvarchar({maxLength})";
                return _dbType == DatabaseSouceType.PostgreSQL ? "text" : "nvarchar(max)";
            }

            if (clrType == typeof(byte[]))
                return _dbType == DatabaseSouceType.PostgreSQL ? "bytea" : "varbinary(max)";

            return _dbType switch
            {
                DatabaseSouceType.PostgreSQL => PgTypeMap.GetValueOrDefault(clrType, "text"),
                DatabaseSouceType.SqlServer => SqlServerTypeMap.GetValueOrDefault(clrType, "nvarchar(max)"),
                _ => "text"
            };
        }
        ...
        private string GenerateCreateTable(string tableName, List<ColumnInfo> columns)
        {
            ...
            foreach (var col in columns)
            {
                ...
                if (col.IsAutoGenerated)
                {
                    ...
                    parts.Add(_dbType == DatabaseSouceType.PostgreSQL
