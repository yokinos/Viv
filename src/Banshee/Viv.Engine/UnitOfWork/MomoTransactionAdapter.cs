...
{
    /// <summary>
    /// 把 <see cref="ITransactionKernel"/> 接到 Momo 现有的三个事务方法上，纯转发零逻辑。
    ///
    /// 工作单元建在 <c>IMomoDbContext</c> 已有的 Begin / Commit / Rollback 之上，不改 Momo 的源码、
    /// 接口、读路由和行为。将来要换内核，换这个类即可。
    ///
    /// 生命周期必须与 <c>IMomoDbContext</c> 对齐（都是 Scoped）：Momo 的事务状态挂在它自己的
    /// <c>_transaction</c> 字段上跟着实例走，适配器若被解析到别的实例（尤其是根作用域），
    /// 开的和提交的会是两个不同的事务。
    /// </summary>
