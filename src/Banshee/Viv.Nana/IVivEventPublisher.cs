...
namespace Viv.Nana
{
    public interface IVivEventPublisher
    {
        ...
        /// <summary>
        /// 发布信封（原样重发）—— 保留 MessageId / Context / ReDeliverCount / CreatedAt，
        /// 且不重新盖 holderId（投递的是一个已经冻结的信封）。供发件箱把持久化过的信封重新投递上线。
        ///
        /// 必须走这个方法而不是内容版：内容版每次新建信封、MessageId 会被重新生成，
        /// 而 MessageId 正是消费端 <c>nana:{ServiceName}:{EventType}:{MessageId}</c> 消费锁的去重键。
        ///
        /// 刻意不叫 PublishAsync 重载：那样与内容版同为一个参数，调用点写 <c>PublishAsync&lt;T&gt;(null)</c>
        /// 时 null 字面量对两个重载都成立且互不更优 → CS0121 二义。换个名字彻底躲开。
        /// </summary>
        /// <typeparam name="T"></typeparam>
